...
import { AddLocation, Payment } from '@material-ui/icons';
...
const Recerpt = (props) => {
    ...
    console.log("id odder: ", props.id.match.params.id)
                        ...
                        <Table aria-label="spanning table">
                            <TableHead>
                                ...
                                <TableRow >
                                    ...
                                    <TableCell align="left" sx={{ color: "gray" }}>Quantity</TableCell>
                                    <TableCell align="left" sx={{ color: "gray" }}>Price</TableCell>
                                    <TableCell align="left" sx={{ color: "gray" }}>Money</TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                    ...
                                    <TableRow key={row.desc}>
                                        <TableCell>
                                            <div style={{ display: "flex" }}>
                                                ...
                                                <span style={{ margin: "auto 10px" }}>{row.desc}</span>
                                            </div>
                                        </TableCell>
    ...
    const handleClickOpen = async () => {
        let url = "api/pay/info-payment/" + props.id.match.params.id + "/"
        try{
            let res = await callApi(url, 'GET', null, null)
            ...
            console.error("Error recerpt.js -> handleClickOpen", err.responses)
        }
        setOpen(true);
    };
    ...
    return (
        <>
            <Grid item xs={10} className="container pb-lg-4" style={{ margin: "100px auto" }}>
                <Paper className={classes.paper} elevation={3}>
                    <div style={{ display: "flex", color: "#ff5200", marginBottom: "7px" }}>
                        <AddLocation />
                        <p style={{ color: "#ff5200", fontSize: "20px" }}>Địa chỉ nhận hàng</p>
                    </div>
                    <div style={{ display: "flex" }}>

                        <div style={{ padding: "0 20px", fontWeight: "700" }}>{`${context.dataProfile.first_name} ${context.dataProfile.last_name} (+84) ${context.dataProfile.phone_number?.slice(1, 10)}`} </div>

                        <div style={{ padding: "0 30px" }}>{context.dataProfile.address}</div>
                ...
                {showPayMent === true ?
                    <Paper className={classes.paper} elevation={3}>
                        <div style={{ display: "flex", color: "#ff5200", marginBottom: "7px" }}>
                            <Payment />
                            <p style={{ color: "#ff5200", fontSize: "20px" }}>Phương thức thanh toán</p>
                        </div>
                        <div style={{ display: "flex" }}>

                            <div style={{ padding: "0 20px", margin: "0 10px", width: "20%", border: "2px blue solid" }}
                            ...
                            <div style={{ padding: "0 20px", margin: "0 10px", width: "25%", border: "2px blue solid" }}>
                                <img
                                    style={{ width: "100%", height: "100px" }}
                                    alt=''
            ...
            >
                <DialogTitle>{"Payment online"}</DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-slide-description">
                        <PayPalScriptProvider options={{ "client-id": data.client_id }}>
                            <PayPal payload={data} auction_item_id={props.id.match.params.id}/>
